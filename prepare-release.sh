...
main() {
    ...
    # Git operations
    echo""
    read -p "Prepare Git commit and tag? (y/N): " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        prepare_git_commit
    fi
    ...
    echo "  1. Review the package in: $DIST_DIR/"
    echo "  2. Test the package on a clean system"
    echo "  3. Push to GitHub and create release"
